...
import {
  authenticate, fetchData, pingMongoDB, pingNode, pingRedis,
  ...
  const onClick = async () => {
    try {
      ...
      if (e.response) {
        ...
        ToastsStore.error(`Saving failed. Error: ${e.response.data.error}`,
          5000);
      } else {
...
function PingNodeButton({ disabled, apiUrl, nodeName }) {
  const onClick = async () => {
    try {
      ToastsStore.info(`Pinging node ${nodeName} at ${apiUrl}`, 5000);
      await pingNode(apiUrl, nodeName);
...
function PingBotButton({ disabled, botToken, botChatID }) {
  const onClick = async () => {
    try {
      ...
      ToastsStore.success(
        'Ping request sent. Make sure to check the chat corresponding with '
        + `chat id ${botChatID} for a PONG!`, 7000,
      );
    } catch (e) {
      if (e.response) {
        // The request was made and the server responded with a status code
        // that falls out of the range of 2xx
        ToastsStore.error(
          `Ping request failed. Error: ${e.response.data.description}`, 5000,
        );
      } else {
        // Something happened in setting up the request that triggered an Error
        ToastsStore.error(`Ping request failed. Error: ${e.message}`, 5000);
...
export {
  ...
  TestCallButton, ConnectWithMongoButton, ConnectWithRedisButton,
