...
import {
  fetchData, pingAPIServer, pingMongoDB, pingNode, pingRedis, pingRepo,
  sendConfig, sendTestEmail, testCall,
} from '../utils/data';
  ...
  return (
    <Button
      ...
      onClick={async () => {
        try {
          ToastsStore.info(
            `Sending test alert. Make sure to check the chat
            with id ${botChatID}`, 5000,
          );
          await fetchData(
