...
- **Optional**: Node Exporter, this will be used to monitor the systems on which nodes are running.
...
- **Node Exporter**: [Click here](INSTALL_NODE_EXPORTER.md) if you want to monitor the system. This must be installed on the machine that the nodes are running on.
...
First start by setting up the UI as described [here](./SETUP_UI.md).
...
You can now proceed by setting up the alerter using [this guide](./SETUP_ALERTER.md).
...
Having installed and executed the necessary components for PANIC, you can now proceed by setting up PANIC using [this guide](./SETUP_ALERTER.md).
